...
def test_mine_subcommand_with_check_trends(capsys):
    ...
    trend_results = [
        TrendResult(
            keyword="hostile co-parent communication",
            ok=True,
            current_interest=20.0,
            change_pct=14.0,
            verdict="RISING",
        )
    ]
    ...
    assert called_categories == ["hostile co-parent communication"]
...
def test_mine_csv_includes_trend_columns(tmp_path):
    search_results, thread, pain_points = _fake_mine_dependencies()
    trend_results = [
        TrendResult(
            keyword="hostile co-parent communication",
